...
const AdminFaculties = () => {
  ...
  const [form, setForm] = useState({ name: "", role: "", subject: "", experience: "" });
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    addFaculty(form);
    setForm({ name: "", role: "", subject: "", experience: "" });
    setShowForm(false);
    refresh();
  };
  ...
  const startEdit = (f: FacultyMember) => {
    setEditId(f.id);
    setForm({ name: f.name, role: f.role, subject: f.subject, experience: f.experience });
    setShowForm(false);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setShowForm(true); setEditId(null); setForm({ name: "", role: "", subject: "", experience: "" }); }} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gold text-secondary-foreground text-sm font-semibold hover:bg-gold-dark transition-colors">
